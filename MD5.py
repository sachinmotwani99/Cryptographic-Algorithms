...
#A function to compute circular shift
...
#Function to divide the message block
...
#Defining the four main functions F,G,H,I
def F(X,Y,Z):
    return( (X&Y)|((~X)&Z) )
...
#The function where the process of hashing occurs
def md5sum(msg):
    #First, we pad the message
    msgLen = bitlen(msg)%(2**64)

    #adding extra 64 bits
    msg = msg + b'\x80'
...
if __name__ == "__main__":
    if len(sys.argv) <= 1:
        ...
        #reading data from file in binary form
        fname = sys.argv[1]
